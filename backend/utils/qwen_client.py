...
# 优先使用DASHSCOPE_API_KEY，兼容旧的QWEN_API_KEY
...
def call_qwen(prompt: str, timeout: int = 60, enable_search: bool = False) -> str:
    """
    调用 Qwen-Max 模型生成文本。
    ...
    data = {
        ...
        "parameters": {
            "temperature": 0.3,
            "enable_search": enable_search # [NEW] Enable Aliyun Native Search
        }
...
async def async_call_qwen(prompt: str, timeout: int = 60, enable_search: bool = False) -> str:
    """
    异步调用 Qwen-Max 模型生成文本。
    ...
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.post(QWEN_URL, json=data, headers=headers, timeout=timeout)
        ...
        print(f"❌ Unexpected error in async_call_qwen: {e}")
        return ""